...
export const validationAndPassthroughCommandOverlays: Array<[string, CommandOverlay]> = [
	...
	['clean', command({
			group: 'Workflow',
			summary: 'Stop this worktree and remove its generated project data.',
			description: 'Stops only the current worktree services through reconciliation, then removes selected generated evidence and caches.',
			provider: 'default',
			usage: 'treeseed clean [project] [--mode standard|aggressive] [--plan|--yes] [--json]',
			arguments: [{ name: 'action', description: 'Project cleanup scope. Defaults to project.', required: false }],
			options: [
				{ name: 'mode', flags: '--mode <mode>', description: 'Cleanup mode.', kind: 'enum', values: ['standard', 'aggressive'] },
				{ name: 'plan', flags: '--plan', description: 'Inspect scoped service and file cleanup without changing state.', kind: 'boolean' },
				{ name: 'yes', flags: '--yes', description: 'Confirm current-worktree service shutdown and project cleanup.', kind: 'boolean' },
				{ name: 'json', flags: '--json', description: 'Emit structured JSON.', kind: 'boolean' },
			],
			examples: ['trsd clean --plan --json', 'trsd clean project --mode aggressive --yes'],
			notes: ['Project cleanup preserves Git state, source, configuration, credentials, databases, active workflow journals, sibling worktrees, and root operator dependencies.', 'Aggressive project cleanup also removes package-local dependencies/build closures and the regenerable local TreeDX index.'],
