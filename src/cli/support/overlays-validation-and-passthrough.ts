...
export const validationAndPassthroughCommandOverlays: Array<[string, CommandOverlay]> = [
	...
	['cleanup', command({
			group: 'Workflow',
			summary: 'Plan or remove generated project data while preserving durable project and operator state.',
			description: 'Plans or removes generated logs, caches, completed scene evidence, exports, build output, and aggressive package build/install artifacts. Source, Git state, configuration, credentials, databases, active workflow journals, and the root operator dependency closure are preserved.',
			provider: 'default',
			usage: 'treeseed cleanup local [--mode standard|aggressive] [--plan|--yes] [--json]',
			...
			options: [
				...
				{ name: 'plan', flags: '--plan', description: 'Inspect exact cleanup targets without deleting them.', kind: 'boolean' },
				{ name: 'yes', flags: '--yes', description: 'Confirm live deletion after inspecting the plan.', kind: 'boolean' },
				{ name: 'json', flags: '--json', description: 'Emit structured JSON.', kind: 'boolean' },
			],
			examples: ['treeseed cleanup local --mode aggressive --plan --json', 'treeseed cleanup local --mode aggressive --yes --json'],
			notes: ['Live cleanup requires --yes. Aggressive cleanup preserves Treeseed config, workflow locks, current journals, package source, databases, and checked-in files.'],
