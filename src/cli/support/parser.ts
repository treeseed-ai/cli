import type { CommandOptionSpec, OperationSpec, ParsedInvocation } from '../operations/operations-types.ts';

function optionKey(spec: CommandOptionSpec) {
	return spec.name;
}

export function parseInvocation(command: OperationSpec, argv: string[]): ParsedInvocation {
	const args: ParsedInvocation['args'] = {};
	const positionals: string[] = [];
	const options = new Map<string, CommandOptionSpec>((command.options ?? []).map((spec) => [spec.name, spec]));
	const byFlag = new Map<string, CommandOptionSpec>();
...
export function validateInvocation(command: OperationSpec, invocation: ParsedInvocation): string[] {
	const errors: string[] = [];
