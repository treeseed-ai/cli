import { findOperation, listOperationNames, TRESEED_OPERATION_SPECS } from '../operations/operations-registry.ts';
import type {
	CommandGroup,
	CommandHelpDetail,
	CommandRelatedDetail,
	OperationSpec,
...
export type HelpEntryAccent = 'command' | 'flag' | 'argument' | 'example' | 'alias' | 'related';

export type HelpEntry = {
	label: string;
	summary?: string;
	accent?: HelpEntryAccent;
	required?: boolean;
...
function formatExecutionMode(spec: OperationSpec) {
...
function formatOptionSummary(spec: OperationSpec, flags: string, description: string) {
...
function automationNotes(spec: OperationSpec) {
...
function normalizeStructuredExamples(spec: OperationSpec): StructuredCommandExample[] {
...
function detailMap(details: CommandHelpDetail[] | undefined) {
	return new Map((details ?? []).map((entry) => [entry.name, entry.detail]));
}

function relatedMap(details: CommandRelatedDetail[] | undefined) {
...
function exampleEntries(spec: OperationSpec): HelpEntry[] {
...
function groupedCommands() {
	const groups = new Map<CommandGroup, OperationSpec[]>();
...
function specEntries(specs: OperationSpec[]): HelpEntry[] {
...
function entryLines(entries: HelpEntry[]) {
	if (entries.length === 0) {
		return [];
	}
...
export function renderUsage(spec: OperationSpec) {
...
export function buildHelpView(commandName?: string | null): HelpView {
	if (!commandName) {
...
export function renderHelp(commandName?: string | null) {
	const view = buildHelpView(commandName);
	const sections = [
		view.title,
		view.subtitle ?? '',
		'',
