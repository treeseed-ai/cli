...
export const scenesAndGuaranteesOperationSpecs: OperationSpec[] = [
	{
			...
			arguments: [
				{ name: 'action', description: 'Scene action. Use status, validate, plan, run, view, visual-audit, inspect, resume, render, training, evidence, publish, publish-plan, or export.', required: false },
			...
			options: [
				...
				{ name: 'agentTest', flags: '--agent-test <ids>', description: 'Override comma-separated Agent Lab workday tests.', kind: 'string', repeatable: true },
				{ name: 'project', flags: '--project <projects>', description: 'Override comma-separated Agent Lab project or repository selectors.', kind: 'string', repeatable: true },
				{ name: 'agent', flags: '--agent <agents>', description: 'Restrict an Agent Lab run to selected content-defined agents.', kind: 'string', repeatable: true },
				{ name: 'agentClass', flags: '--agent-class <classes>', description: 'Restrict an Agent Lab run to selected project agent classes.', kind: 'string', repeatable: true },
				{ name: 'presentation', flags: '--presentation <adapter>', description: 'Select race-control, strategy-command, or esports-tournament Agent Lab presentation.', kind: 'enum', values: ['race-control', 'strategy-command', 'esports-tournament'] },
				{ name: 'timeZone', flags: '--time-zone <iana-zone>', description: 'Override the normalized IANA time zone embedded in Agent Lab evidence.', kind: 'string' },
				{ name: 'open', flags: '--open', description: 'Open the loopback Agent Lab report when it becomes available.', kind: 'boolean' },
			...
			examples: [
				...
				'trsd scene view <run-id-or-path> --open',
		...
		},
	{
			...
			usage: 'treeseed guarantees [status|catalog-status|campaign-status|preflight] [--json]\n       treeseed guarantees watch --market local --workday <id> --generation <digest> [--json]\n       treeseed guarantees proof-capture --id <id> --variant <variant> --input <recipe> --output <proof> --team <team> --assignment <id> --agent-test <id> [--json]\n       treeseed guarantees campaign-run --file <manifest> --execute [--json]\n       treeseed guarantees campaign-cleanup --campaign <id> --execute [--json]\n       treeseed guarantees validate [--type <type>] [--subtype <subtype>] [--gate <gate>] [--json]\n       treeseed guarantees proof-template --id <id> --variant <variant> --output <path> [--json]\n       treeseed guarantees proof-rebase --id <id> --variant <variant> --input <path> --output <path> --assignment <id> --workday <id> [--artifact-path <path>] [--json]\n       treeseed guarantees proof-validate --id <id> --variant <variant> --proof-input <path> [--json]\n       treeseed guarantees audit-journeys [--owner-package <package>] [--write-report <path>] [--json]\n       treeseed guarantees plan [--environment local|staging|prod] [--type <type>] [--subtype <subtype>] [--gate <gate>] [--json]\n       treeseed guarantees export [--format csv|json|markdown] [--output <path>] [--type <type>] [--subtype <subtype>] [--json]\n       treeseed guarantees run [--environment local|staging|prod] [--id <id>] [--prove-planned --variant baseline|clean-repeat|interruption-resume] [--proof-input <path>] [--json]',
			arguments: [
				{ name: 'action', description: 'Guarantee action. Use status, catalog-status, preflight, validate, proof-template, proof-validate, audit-journeys, plan, export, or run.', required: false },
			],
			options: [
				...
				{ name: 'provePlanned', flags: '--prove-planned', description: 'Execute selected planned v2 catalog guarantees as activation candidates.', kind: 'boolean' },
				{ name: 'variant', flags: '--variant <variant>', description: 'Name the activation run variant.', kind: 'enum', values: ['baseline', 'clean-repeat', 'interruption-resume'] },
				{ name: 'proofInput', flags: '--proof-input <path>', description: 'Exact agent proof input containing CLI commands, semantic assertions, repository refs, and cleanup read-back.', kind: 'string' },
				{ name: 'input', flags: '--input <path>', description: 'Existing exact proof recipe to rebase onto a new assignment and workday.', kind: 'string' },
				{ name: 'assignment', flags: '--assignment <assignment-id>', description: 'Exact assignment identity used when rebasing a proof recipe.', kind: 'string' },
				{ name: 'workday', flags: '--workday <workday-id>', description: 'Exact workday identity used when rebasing a proof recipe.', kind: 'string' },
				{ name: 'artifactPath', flags: '--artifact-path <path>', description: 'Semantically verified exact artifact path used when rebasing a proof recipe.', kind: 'string' },
				{ name: 'agentTest', flags: '--agent-test <id>', description: 'Immutable semantic agent-test identity used for authoritative proof capture.', kind: 'string' },
				{ name: 'team', flags: '--team <team>', description: 'Exact team id or slug for assignment proof capture.', kind: 'string' },
				{ name: 'market', flags: '--market <profile>', description: 'Authenticated Market API profile.', kind: 'string' },
				{ name: 'generation', flags: '--generation <digest>', description: 'Pinned canonical agent guarantee generation.', kind: 'string' },
				{ name: 'timeoutSeconds', flags: '--timeout-seconds <seconds>', description: 'Bounded watcher or campaign command deadline.', kind: 'number' },
				{ name: 'pollIntervalMs', flags: '--poll-interval-ms <milliseconds>', description: 'Bounded durable-state watcher interval.', kind: 'number' },
				{ name: 'file', flags: '--file <path>', description: 'Workspace-relative three-variant campaign manifest.', kind: 'string' },
				{ name: 'campaign', flags: '--campaign <id>', description: 'Exact campaign id for replayable cleanup.', kind: 'string' },
				{ name: 'execute', flags: '--execute', description: 'Execute a validated campaign or cleanup journal.', kind: 'boolean' },
				{ name: 'catalog', flags: '--catalog <catalog>', description: 'Filter catalog-status to a stable catalog id.', kind: 'string' },
