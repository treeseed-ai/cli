...
const CLI_ONLY_OPERATION_SPECS: TreeseedOperationSpec[] = [
	...
	},
	{
		...
		help: {
			...
			automationNotes: ['Use `--json` for stable output. Assignment, mode-run, and assignment-explanation inspection includes read-only execution visibility and capability match summaries. Use `--plan` for lifecycle previews and `--execute` for mutations. Registration-key reveal and rotation additionally require `--yes` in noninteractive automation.'],
		},
		helpVisible: true,
		helpFeatured: false,
		executionMode: 'handler',
