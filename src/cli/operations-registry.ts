...
const CLI_ONLY_OPERATION_SPECS: TreeseedOperationSpec[] = [
	...
	},
	{
		...
		usage: 'treeseed hosting [status|plan|apply|verify] [--environment local|staging|prod] [--service api] [--json]',
		...
		examples: [
			...
			'treeseed hosting apply --environment staging --service marketOperationsRunner --execute --json',
		],
		help: {
	...
	},
	{
		...
		description: 'Runs deployment graph readiness, hosted service checks, and Market operations runner smoke diagnostics before expensive workflow promotion.',
		provider: 'default',
	...
	},
	{
		...
		aliases: [],
		group: 'Utilities',
		summary: 'Run Market operation-runner diagnostics.',
		...
		usage: 'treeseed operations smoke [--environment staging|prod] [--service marketOperationsRunner] [--json]',
		...
		options: [
			...
			{ name: 'service', flags: '--service <service-id>', description: 'Service to smoke test. Currently marketOperationsRunner.', kind: 'string' },
			{ name: 'json', flags: '--json', description: 'Emit machine-readable JSON instead of human-readable text.', kind: 'boolean' },
		],
		examples: [
		...
		help: {
			longSummary: ['Operations smoke proves that the Market API, database, and operations runner work together before TreeDX/bootstrap workflows queue real work.'],
			whenToUse: ['Run when staging deploys time out with queued operations or before release resource verification.'],
			beforeYouRun: ['Run after the API and operations runner are deployed. Ensure the selected environment has the Market API base URL and service credentials configured through Treeseed config.'],
	...
	},
	{
		...
		description: 'Inspect, provision, connect, mirror, share, publish, and bind TreeDX libraries for Market projects.',
		provider: 'default',
		...
		help: {
			longSummary: ['TreeDX commands manage the Market control-plane records that bind each team to one primary knowledge-library instance and bind project content to TreeDX libraries.'],
			whenToUse: ['Use this when provisioning a private TreeDX, attaching a public team to federation, creating mirror/share records, or inspecting project content/site/project topology.'],
			beforeYouRun: ['Log in to the selected Market first. Do not pass secrets on the command line; TreeDX credentials belong in host secret managers or encrypted config.'],
			automationNotes: ['Use `--json` for stable TreeDX binding, mirror, share, library, and topology records.'],
		},
		helpVisible: true,
		helpFeatured: false,
		executionMode: 'handler',
