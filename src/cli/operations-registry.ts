...
const CLI_ONLY_OPERATION_SPECS: TreeseedOperationSpec[] = [
	...
	},
	{
		...
		usage: 'treeseed hosting [status|plan|apply|verify] [--environment local|staging|prod] [--json]',
		...
		examples: [
			...
			'treeseed hosting apply --environment staging --dry-run --json',
		],
		help: {
			...
			whenToUse: ['Use this before dev, stage, or release when you need to inspect host bindings, missing capabilities, or TreeDB/public federation placement.'],
	...
	},
	{
		...
		aliases: [],
		group: 'Utilities',
		summary: 'Manage team TreeDB knowledge-library bindings.',
		description: 'Inspect, provision, connect, mirror, share, publish, and bind TreeDB libraries for Market projects.',
		provider: 'default',
		related: ['teams', 'projects', 'capacity'],
		usage: 'treeseed db [status|provision|connect|mirrors|shares|library|topology|publish]',
		arguments: [{ name: 'action', description: 'TreeDB action.', required: false }],
		options: [
			{ name: 'market', flags: '--market <id-or-url>', description: 'Select a configured market id or direct market API URL.', kind: 'string' },
			...
			{ name: 'selfHosted', flags: '--self-hosted', description: 'Connect a self-hosted TreeDB URL as the primary binding.', kind: 'boolean' },
			{ name: 'url', flags: '--url <url>', description: 'TreeDB base URL for connect/provision.', kind: 'string' },
			{ name: 'registryUrl', flags: '--registry-url <url>', description: 'Optional TreeDB registry URL.', kind: 'string' },
			{ name: 'image', flags: '--image <image-ref>', description: 'TreeDB container image ref, default treeseed/treedb:latest.', kind: 'string' },
			...
			{ name: 'targetKind', flags: '--target-kind <kind>', description: 'Mirror target kind such as git or treedb.', kind: 'string' },
			...
			{ name: 'library', flags: '--library <library-id>', description: 'TreeDB library id for project binding or share.', kind: 'string' },
			{ name: 'instance', flags: '--instance <instance-id>', description: 'TreeDB instance id for project binding.', kind: 'string' },
			{ name: 'repository', flags: '--repository <repo-id>', description: 'TreeDB repository id for project binding.', kind: 'string' },
		...
		examples: [
			...
			'treeseed db connect --team team_123 --url https://treedb.example.com --self-hosted',
			'treeseed db mirrors --team team_123 --name customer-copy --target-url https://treedb.customer.example',
		...
		help: {
			longSummary: ['TreeDB commands manage the Market control-plane records that bind each team to one primary knowledge-library instance and bind project content to TreeDB libraries.'],
			whenToUse: ['Use this when provisioning a private TreeDB, attaching a public team to federation, creating mirror/share records, or inspecting project content/site/project topology.'],
			beforeYouRun: ['Log in to the selected Market first. Do not pass secrets on the command line; TreeDB credentials belong in host secret managers or encrypted config.'],
			automationNotes: ['Use `--json` for stable TreeDB binding, mirror, share, library, and topology records.'],
		},
		helpVisible: true,
		helpFeatured: false,
		executionMode: 'handler',
