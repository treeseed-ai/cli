...
const CLI_ONLY_OPERATION_SPECS: TreeseedOperationSpec[] = [
	...
	},
	{
		...
		options: TOOL_WRAPPER_OPTIONS,
		examples: ['treeseed docker --environment staging -- buildx imagetools inspect treeseed/agent-api:dev-staging', 'treeseed docker --environment staging -- manifest inspect treeseed/agent-runner:dev-staging'],
		help: {
			...
			beforeYouRun: ['Run from a Treeseed project. Use `--environment staging` for staging image checks.'],
	...
	},
	{
		...
		usage: 'treeseed capacity [doctor|register|plan|migrate|allocation-sets|agent-classes|provider-sessions|assignments|mode-runs|execution-runs|decision-planning|execution-inputs|capacity-plans|capacity-plan|workday|workday-summary|workday-test|assignment-explanation|fallback-outputs|treedx-proxy-audit|up|down|restart|logs|status|build|test-local]',
		...
		options: [
			...
			{ name: 'projects', flags: '--projects <all|csv>', description: 'Project slug selector for portfolio workday tests.', kind: 'string' },
			{ name: 'scenario', flags: '--scenario <id>', description: 'Workday test scenario id.', kind: 'string' },
			{ name: 'workdays', flags: '--workdays <count>', description: 'Number of workdays to exercise in a workday test.', kind: 'string' },
			{ name: 'maxAssignments', flags: '--max-assignments <count>', description: 'Maximum assignments to synthesize or inspect during a workday test.', kind: 'string' },
			{ name: 'planningOnly', flags: '--planning-only', description: 'Limit workday test expectations to planning mode.', kind: 'boolean' },
			{ name: 'reportDir', flags: '--report-dir <path>', description: 'Directory for generated workday test JSON and Markdown reports.', kind: 'string' },
			...
			{ name: 'format', flags: '--format <text|yaml>', description: 'Render selected inspection commands as text or YAML.', kind: 'string' },
			...
			{ name: 'waitSeconds', flags: '--wait-seconds <seconds>', description: 'Seconds for workday-test to wait for provider manager and runner lease consumption before scoring.', kind: 'string' },
		...
		examples: [
			...
			'treeseed capacity workday-test --market local --team team_123 --provider local --workdays 1 --execute --wait-seconds 90 --json',
	...
	},
	{
		...
		usage: 'treeseed reconcile [plan|status|verify|apply|destroy|test-live] [--environment local|staging|prod] [--execute] [--json]',
	...
	},
	{
		...
		options: [
			...
			{ name: 'workflow', flags: '--workflow <file>', description: 'Workflow file name or path, such as dev-image.yml.', kind: 'string' },
		...
		examples: [
			'treeseed workflow dispatch --repo treeseed-ai/agent --workflow dev-image.yml --branch staging --plan --json',
			'treeseed workflow dispatch --repo treeseed-ai/agent --workflow dev-image.yml --branch staging --execute --json',
			'treeseed workflow dispatch --repo treeseed-ai/treedx --workflow dev-image.yml --branch staging --input imageTag=dev-staging --execute --json',
		],
		help: {
			...
			whenToUse: ['Use this to test package image workflows, release-gate workflows, or no-op diagnostic workflows from the CLI when a push-triggered run is unnecessary.'],
	...
	},
	{
		...
		description: 'Validate package manifests and plan, sync credentials for, or dispatch package development image workflows declared by treeseed.package.yaml.',
		provider: 'default',
