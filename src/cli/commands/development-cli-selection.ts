...
export function selectDevelopmentCli(env: NodeJS.ProcessEnv, selection: { entrypoint: string } | null) {
	const path = developmentCliEntrypointPath(env);
	if (selection === null) { rmSync(path, { force: true }); return; }
	const resolved = resolve(selection.entrypoint);
	...
	writeFileSync(temporary, `treeseed.development-cli-selection/v2\n${resolved}\n`, { mode: 0o600 });
