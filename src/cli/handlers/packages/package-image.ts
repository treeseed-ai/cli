...
export async function runPackageImageCommand(
	...
	const resolvedEnv = resolveLaunchEnvironment({
		tenantRoot: context.cwd,
		scope: 'staging',
		baseEnv: context.env,
	});
	const { imagePlan, selectedWorkflow, dockerHub, credential, report } = await runPackageImageWorkflow({
		root: context.cwd,
		packageId,
		branch,
		workflow,
	...
	if (syncConfig || execute) {
		...
		report.ok = execute
			? (reconcile as Awaited<ReturnType<typeof reconcileTarget>>).ok
			: !(reconcile as Awaited<ReturnType<typeof planReconciliation>>).plans.some((plan) => plan.diff.action === 'blocked');
