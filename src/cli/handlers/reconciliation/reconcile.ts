import {
	...
	type CapacityAcceptanceExecutionInput,
	type LiveReconcileEnvironment,
	type LiveReconcileMode,
...
function environmentFor(value: unknown): LiveReconcileEnvironment {
...
function modeFor(value: unknown): LiveReconcileMode {
...
function providersFor(value: unknown): LiveReconcileProvider[] {
	...
	return [...new Set(providers)] as LiveReconcileProvider[];
}
...
function targetFor(environment: LiveReconcileEnvironment): ReconcileTarget {
...
function selectorFor(args: Record<string, unknown>, environment: LiveReconcileEnvironment): ReconcileSelector | undefined {
	const selector: ReconcileSelector = { environment };
	const assign = <Key extends keyof ReconcileSelector>(key: Key, value: unknown) => {
		const values = stringList(value);
		if (values.length > 0) {
			selector[key] = values as ReconcileSelector[Key];
		}
	};
...
function renderScenarioRows(result: Awaited<ReturnType<typeof runLiveReconcileTests>>) {
...
function graphSummary(desiredGraph: ReturnType<typeof compileDesiredResourceGraph>) {
...
function resultSummary(result: Awaited<ReturnType<typeof reconcileTarget>>) {
...
export const handleReconcile: CommandHandler = async (invocation, context) => {
	try {
		...
		if (subcommand === 'plan' || subcommand === 'status' || subcommand === 'verify' || subcommand === 'apply' || subcommand === 'destroy') {
			const resolvedEnv = {
				...context.env,
				...collectConfigSeedValues(context.cwd, environment, context.env),
			};
			const desiredGraph = compileDesiredResourceGraph({ tenantRoot: context.cwd, target });
			const selector = selectorFor(invocation.args, environment);
			const desiredUnits = compileDesiredUnitsFromGraph(desiredGraph, selector);
			if (subcommand === 'plan') {
				const planned = await planReconciliation({ tenantRoot: context.cwd, target, env: resolvedEnv, units: desiredUnits });
			...
			if (subcommand === 'status' || subcommand === 'verify') {
				const status = await collectReconcileStatus({ tenantRoot: context.cwd, target, env: resolvedEnv, units: desiredUnits });
				return guidedResult({
			...
			if (subcommand === 'destroy') {
				...
				return guidedResult({
					...
					report: { target, desiredGraph: graphSummary(desiredGraph), result: resultSummary(result as Awaited<ReturnType<typeof reconcileTarget>>) },
				});
			}
		}
		...
		const capacityAssignmentExecutor = environment === 'local' && providers.includes('local') && mode !== 'smoke'
			? async (input: CapacityAcceptanceExecutionInput) => {
