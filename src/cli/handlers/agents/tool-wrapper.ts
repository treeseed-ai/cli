...
type EnvironmentScope = 'local' | 'staging' | 'prod';
...
function wrapperScope(value: unknown): EnvironmentScope {
...
function railwayEnvironmentName(scope: EnvironmentScope) {
...
function railwayProjectIdFromDeployState(cwd: string, scope: EnvironmentScope) {
...
export const handleToolWrapper: CommandHandler = async (invocation, context) => {
	...
	try {
		...
		const managedEnv = createManagedToolEnv({
			...process.env,
			...(context.env ?? {}),
			...launchEnv,
			TREESEED_ACTIVE_ENVIRONMENT: scope,
		});
		const resolved = resolveToolCommand(toolName, { env: managedEnv });
		if (!resolved) {
			return {
				exitCode: 1,
				stderr: [
		...
		if (toolName === 'railway' && scope !== 'local' && railwayCommandNeedsProjectContext(targetArgs)) {
			...
			const railwayCwd = railwayCommandUsesProjectFiles(targetArgs)
				? context.cwd
				: (isolatedRailwayCwd = mkdtempSync(join(wrapperTempRoot(context.cwd), `treeseed-railway-${scope}-`)));
