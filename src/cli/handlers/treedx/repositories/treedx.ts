...
import { runPackageImageCommand } from '../../packages/package-image.js';
import { fail, guidedResult } from '../../utilities/utils.js';

function stringArg(invocation: ParsedInvocation, key: string) {
	const value = invocation.args[key];
	return typeof value === 'string' && value.trim() ? value.trim() : null;
}

function boolArg(invocation: ParsedInvocation, key: string) {
...
function teamId(invocation: ParsedInvocation) {
	return stringArg(invocation, 'team');
}

function projectId(invocation: ParsedInvocation) {
	return stringArg(invocation, 'project');
}

function environmentArg(invocation: ParsedInvocation) {
...
function jsonResult(invocation: ParsedInvocation, context: unknown, report: Record<string, unknown>) {
...
async function connect(invocation: ParsedInvocation, context: Parameters<CommandHandler>[1]) {
	const team = teamId(invocation);
...
export const handleTreeDx: CommandHandler = async (invocation, context) => {
