...
function mutationHeaders(invocation: TreeseedParsedInvocation) {
...
function collectionQuery(invocation: TreeseedParsedInvocation, filters: Record<string, string | null> = {}) {
...
function numberArgument(invocation: TreeseedParsedInvocation, name: string) {
...
function listArgument(invocation: TreeseedParsedInvocation, name: string) {
	const value = argument(invocation, name);
	return value ? [...new Set(value.split(',').map((entry) => entry.trim()).filter(Boolean))] : [];
}

async function documentArgument(invocation: TreeseedParsedInvocation, context: TreeseedCommandContext) {
...
function grantCreateBody(invocation: TreeseedParsedInvocation) {
	return {
...
export async function runCapacityGovernanceAction(action: string, invocation: TreeseedParsedInvocation, context: TreeseedCommandContext) {
