import type { CiResult } from '@treeseed/sdk/workflow';
...
function failureLines(payload: CiResult) {
...
function pendingLines(payload: CiResult) {
...
function missingOrNotPushedLines(payload: CiResult) {
	const lines: string[] = [];
...
function passingLines(payload: CiResult) {
	return payload.repositories.flatMap((repo) =>
		repo.workflows
...
function ciSections(payload: CiResult, failedOnly: boolean) {
	const sections = [
...
export const handleCi: CommandHandler = async (invocation, context) => {
	try {
		...
		const payload = result.payload as CiResult;
		return guidedResult({
