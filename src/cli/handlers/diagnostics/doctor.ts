...
import { collectCliPreflight, collectDeploymentReadiness, collectHostedServiceChecks } from '@treeseed/sdk/workflow-support';
import { guidedResult } from '../utilities/utils.js';
import { applySafeRepairs } from '../../support/repair.js';
import { createWorkflowSdk, workflowErrorResult } from '../operations/workflow.js';

export const handleDoctor: CommandHandler = async (invocation, context) => {
	try {
	const status = await createWorkflowSdk(context).status();
	const state = status.payload;
	const hostedServices = invocation.args.live === true || invocation.args.hostedServices === true
	...
	const deploymentReadiness = invocation.args.live === true || invocation.args.hostedServices === true
		? collectDeploymentReadiness({ tenantRoot: context.cwd, environment: hostedTarget })
		: null;
	const performedFixes = invocation.args.fix === true && state.deployConfigPresent
		? applySafeRepairs(state.cwd)
		: [];
