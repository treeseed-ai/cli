import { recoverGitLocks } from '@treeseed/sdk/workflow-support';
import type { GitLockDiagnostic } from '@treeseed/sdk/workflow-support';
...
function flattenGitLockDiagnostics(diagnostic: ReturnType<typeof recoverGitLocks>): GitLockDiagnostic[] {
	...
	return diagnostic.repositories.flatMap((entry) => {
		...
		return [entry as GitLockDiagnostic];
	});
}

function gitLockLabel(entry: GitLockDiagnostic) {
	return `${entry.kind} lock ${entry.relativeLockPath ?? entry.lockPath ?? '(unknown path)'}`;
}

export const handleRecover: CommandHandler = async (invocation, context) => {
	try {
		if (invocation.args.gitLocks === true || invocation.args['git-locks'] === true) {
			const execute = invocation.args.execute === true;
			const diagnostic = recoverGitLocks(context.cwd, { execute, all: true });
