...
import { applyEnvironmentToProcess } from '@treeseed/sdk/workflow-support';
import {
...
import { classifyGitMode, runGitText } from '@treeseed/sdk/operations';
import { repoRoot } from '@treeseed/sdk/workflow-support';
import { guidedResult } from '../utilities/utils.js';
import { copyOperationalState } from '../../support/repair.js';
...
function runGitWorktree(repoDir: string, args: string[]) {
	...
	return runGitText(gitArgs, { cwd: repoDir, mode: classifyGitMode(gitArgs) });
}

export const handleRollback: CommandHandler = (invocation, context) => {
	...
	try {
		...
		const buildResult = context.spawn(process.execPath, [packageScriptPath('build/tenant-build')], {
			cwd: tempTenantRoot,
			env: { ...context.env },
			stdio: 'inherit',
		});
