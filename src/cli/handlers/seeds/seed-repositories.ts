...
export const handleSeedRepositories: CommandHandler = async (invocation, context) => {
	...
	if (!live) {
		const planned = await planReconciliation({ tenantRoot: context.cwd, target, env: context.env, units: compiled.units });
		...
		return {
			...
			report: { command: 'seed repositories', ok: !plans.some((plan) => plan.action === 'blocked'), mode: 'plan', seed: seedName, environment: selectedEnvironment, manifestPath: compiled.manifestPath, plans },
		};
	}
	const reconciled = await reconcileTarget({ tenantRoot: context.cwd, target, env: context.env, units: compiled.units, write: (line) => context.write(`[seed repositories] ${line}`, 'stderr') });
	...
	return {
		...
		report: { command: 'seed repositories', ok, mode: 'apply', seed: seedName, environment: selectedEnvironment, manifestPath: compiled.manifestPath, plans, results: reconciled.results.map((result) => ({ unitId: result.unit.unitId, action: result.action, verification: result.verification })) },
	};
};
