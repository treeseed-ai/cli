...
import {
	collectLiveHostedServiceChecks as collectLiveChecks,
...
export const handleHosting: CommandHandler = async (invocation, context) => {
		try {
		...
		if (subcommand === 'destroy') {
			...
			if (subcommand === 'plan' || subcommand === 'verify') {
				const plan = await planHostingGraph({ tenantRoot: context.cwd, environment, appId, env, ...filterInput });
				const graph = compileHostingGraph({ tenantRoot: context.cwd, environment, appId, env, ...filterInput });
				const selector = selectorFromHostingGraph(graph);
