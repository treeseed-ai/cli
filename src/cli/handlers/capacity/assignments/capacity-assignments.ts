...
export const CAPACITY_ASSIGNMENT_ACTIONS = new Set(['assignment-authority-probe','assignment-cancel', 'assignment-requeue']);
...
export async function runCapacityAssignmentAction(action: string, invocation: ParsedInvocation, context: CommandContext) {
	...
	if(action==='assignment-authority-probe') {
		const response=await client.assignmentAuthorityProbe(teamId,assignmentId);
		return guidedResult({ command:'capacity assignment-authority-probe',summary:`Evaluated frozen authority for assignment ${assignmentId}.`,
			facts:[{label:'Market',value:`${profile.id} (${profile.baseUrl})`},{label:'Team',value:teamId},{label:'Assignment',value:assignmentId}],
			report:{ action,teamId,assignmentId,payload:response.payload } });
	}
