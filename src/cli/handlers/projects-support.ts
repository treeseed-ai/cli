...
export function stringArg(invocation: TreeseedParsedInvocation, key: string) {
	const value = invocation.args[key];
	return typeof value === 'string' && value.trim() ? value.trim() : null;
}

export function boolArg(invocation: TreeseedParsedInvocation, key: string) {
	return invocation.args[key] === true;
}

export function environmentArg(invocation: TreeseedParsedInvocation): ProjectDeploymentEnvironment {
...
export function timeoutSeconds(invocation: TreeseedParsedInvocation) {
	const value = Number(stringArg(invocation, 'timeoutSeconds') ?? 300);
	return Number.isFinite(value) && value > 0 ? value : 300;
}

export function pollIntervalMs(invocation: TreeseedParsedInvocation) {
...
export async function handleProjectImport(invocation: TreeseedParsedInvocation, context: TreeseedCommandContext) {
...
export function deploymentRequestBody(invocation: TreeseedParsedInvocation, action: ProjectWebDeploymentAction, environment: ProjectDeploymentEnvironment) {
