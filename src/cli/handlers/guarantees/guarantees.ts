...
function loadApiAcceptanceServiceEnv(environment: string) {
	...
	const configured = collectConfigSeedValues(process.cwd(), environment, process.env);
...
export const handleGuarantees: CommandHandler = async (invocation, context) => {
	...
	if (action === 'status') {
		const report = createGuaranteeStatusReport({ workspaceRoot: context.cwd });
		return {
	...
	if (action === 'plan') {
		...
		const report = planGuarantees({ workspaceRoot: context.cwd, filter, environment, includeDependencies: invocation.args.dependencies === false || invocation.args.noDependencies === true ? false : undefined });
		return {
			exitCode: report.ok ? 0 : 1,
			stdout: report.ok
				? [
	...
	if (action === 'audit-journeys') {
		const writeReport = typeof invocation.args.writeReport === 'string'
			...
			: typeof invocation.args.output === 'string'
				...
				: undefined;
		const audit = auditGuaranteeJourneys({ workspaceRoot: context.cwd, filter, writeReport });
		return {
	...
	if (action === 'export') {
		...
		const registry = discoverGuarantees({ workspaceRoot: context.cwd, filter });
		if (output) {
			const result = writeGuaranteesExport({ workspaceRoot: context.cwd, output, format: format as 'csv' | 'json' | 'markdown', filter });
			return {
		...
		const content = format === 'csv'
			? exportGuaranteesCsv({ guarantees: registry.guarantees, filter })
			: format === 'json'
				? `${JSON.stringify(exportGuaranteesJson({ registry, filter }), null, 2)}\n`
				: exportGuaranteesMarkdown({ registry, filter });
