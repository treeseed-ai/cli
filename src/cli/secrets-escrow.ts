...
type SecretCapabilityStateInput = Partial<TreeseedClientEncryptedEscrowEnvelopeInput> & {
...
export function summarizeCliClientEncryptedEscrow(record: TreeseedClientEncryptedEscrowEnvelopeInput, now = new Date()) {
...
export function buildCliGitHubActionsSecretDeploymentBody(input: TreeseedGitHubActionsEncryptedSecretDeployment & Record<string, unknown>) {
	return {
...
export function summarizeCliSecretCapabilityState(input: SecretCapabilityStateInput, now = new Date()) {
	...
	const escrow = custodyMode === 'client_encrypted_escrow'
		? summarizeCliClientEncryptedEscrow(input as TreeseedClientEncryptedEscrowEnvelopeInput, now)
		: null;
