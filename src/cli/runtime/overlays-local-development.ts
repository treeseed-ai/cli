	...
	['dev', command({
			...
			help: {
				...
				beforeYouRun: [
					...
					'From the Market repo root, dev automatically starts the local API, managed PostgreSQL, and every declared local capacity provider alongside the web UI.',
					'Use `dev start` for worktree-scoped background instance management that AI agents can discover and stop later.',
					'Use `--plan --json` to inspect web/API and capacity-provider resources, setup steps, readiness checks, watched paths, and restart policy without starting services.',
				...
				examples: [
					example('treeseed dev', 'Start local Market development', 'Run web, API, managed PostgreSQL, and declared agent/platform capacity providers locally.'),
					...
					example('treeseed dev --web-runtime local --plan --json', 'Inspect local web runtime', 'Plan web/API and declared capacity-provider startup using the local Astro web runtime.'),
					example('treeseed dev --port 4322', 'Change the web port', 'Start the web/API and capacity-provider runtime with the Astro UI on a specific port.'),
				...
				outcomes: [
					'Starts local web/API surfaces and every declared capacity provider, waits for readiness, prints the local URL, and remains attached as the live supervisor.',
