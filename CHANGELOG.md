...
- build(build): checkpoint release recovery fixes (ba1dbd1da4fa)
...
- build(build): prevent stale stage auto-resume after candidate heads (423ef85c682f)
- build(build): allow retained production aliases during staging (22cd4d688493)
- build(build): deduplicate Railway API services and enforce canonical (0af077945d1b)

## [0.12.55] - 2026-07-14

### Fixed

- build(build): fix release image state and atomic lockfiles (d4b87ecb982d)

## [0.12.54] - 2026-07-13

### Fixed

- build(build): scope release validation to selected packages (d6f204fb905a)
- build(build): fix selective release metadata for verification-only (4af8387277e5)
...
- build(source): Fix release readiness and merged branch cleanup (97c09804013b)

### Tests

- test(tests): Align CLI stage cleanup contract (84e6641fac63)

### Dependencies

- build(build): Retry transient GitHub job setup failures once (2d1e55c1dc59)
- build(build): Avoid repeated fetches during merged branch cleanup (fdf9cda6c954)
- build(build): Delete merged feature branches after successful stage (4571578869f6)
- build(build): Require explicit workflow resume and prevent stale (187ef4f18320)
- build(build): Permanently isolate Railway staging Git sources (249e9a0c358a)

## [0.12.49] - 2026-07-13

### Dependencies

- build(deps): update dependencies to git references (030fbdff218b)
...
- build(build): preserve registry dependency lock semantics (29ccb8eead7c)

## [0.12.43] - 2026-07-12

### Dependencies

- build(build): reuse exact staged candidate proof during release (042fc932af22)
- build(build): reuse staged package closure during release verification (7f6182621e46)
- build(build): resolve managed release tooling and stale checkout (9e813f3716a8)
...
- build(build): sync starter and fixture submodule promotion with stage (22b7145ca313)

## [0.12.38] - 2026-07-06

### Fixed

- build(build): fix Railway Dockerfile hosted build command verification (a830cabddfcd)

### Tests

- build(build): complete starter, api guarantee, and agent live (07b966ce9607)
- build(deps): complete starter, api guarantee, and agent live (bab1cdd81a90)
...
- build(build): add final production release guarantee gate (3bb18480ad7a)
...
- build(build): bypass source page edge cache for production release (3710f299467b)
- build(build): make live hosted env checks provider authoritative (c2d2cd0ab933)
- build(build): fail release on broken production web surface (1bebd3570a72)

## [0.12.33] - 2026-07-04

### Dependencies

- build(build): purge production web cache before release verification (eb113446a897)
- build(build): purge production web cache before release verification (ee757cdf5cab)
