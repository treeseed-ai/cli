...
test('AI storage binds exact team/node authority and explicit connection/bucket', async () => {
	...
	const exit = await runCommandLine(['ai', 'storage', 'connect', '--team', '11111111-1111-4111-8111-111111111111', '--node', node, '--connection', connection,
	...
	assert.equal(exit, 0, output.join('\n'));
	assert.deepEqual(calls, [{ operationId: 'ai.instances.storage.put', input: { path: { teamId: '11111111-1111-4111-8111-111111111111', instanceId: node }, query: {}, body: { connectionId: connection, bucket: 'test-ai-artifacts' } } }]);
});
