...
test('recovery matches exact session, cwd and invocation, rejecting duplicates', () => {
	...
	const process = { ...expected, pid: 11, argv: ['/usr/bin/node', 'watch.js'] };
	assert.equal(matchDevelopmentProcess([process], expected)?.pid, 11);
	for (const changed of [{ sessionId: 'dev-other' }, { cwd: '/another' }, { argv: ['/usr/bin/node', 'other.js'] }])
		assert.equal(matchDevelopmentProcess([{ ...process, ...changed }], expected), undefined);
	assert.throws(() => matchDevelopmentProcess([process, { ...process, pid: 12 }], expected), /Ambiguous/);
});
...
test('recovery plan does not write, apply preserves current session, missing process fails closed', () => {
	...
	try {
		...
		const plan = planDevelopmentRecovery(record, env, [{ pid: 11, cwd: worktree, worktree, sessionId: 'dev-one', argv: ['/usr/bin/node', 'watch.js'] }]);
