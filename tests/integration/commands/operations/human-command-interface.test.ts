...
function context(fetchCalls: Array<{ url: string; method: string }>, cwd = process.cwd()) {
	const writes: string[] = [];
	return {
		writes,
		overrides: {
			cwd, interactiveUi: false,
...
test('mutations default to execute but fail closed without confirmation', async () => {
	const standaloneRoot = await mkdtemp(join(tmpdir(), 'treeseed-cli-human-interface-'));
	try {
		const run = context([], standaloneRoot);
		const exitCode = await runCommandLine([
			'workdays', 'start', '--market', 'local', '--team', 'team-1',
			'--preflight', 'preflight-1', '--digest', 'sha256:abc', '--json',
		], run.overrides);
