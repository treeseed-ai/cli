...
test('capacity diagnostics reads Market derived capacity projection', async () => {
	...
	process.env[MACHINE_KEY_PASSPHRASE_ENV] = 'test-passphrase';
	unlockSecretSessionFromEnv(root);
	...
	try {
		...
		if (previousPassphrase === undefined) delete process.env[MACHINE_KEY_PASSPHRASE_ENV];
		else process.env[MACHINE_KEY_PASSPHRASE_ENV] = previousPassphrase;
	}
});
...
test('capacity inspection exposes read-only execution visibility summaries', () => {
	const capacityHandler = readFileSync(
		resolve(cliPackageRoot, 'src/cli/handlers/capacity/capacity-core/capacity-inspection-projection.ts'),
		'utf8',
	);
	const operationsRegistry = readFileSync(
		resolve(cliPackageRoot, 'src/cli/capacity/capacity-core/operations-capacity.ts'),
		'utf8',
	);
...
test('cli command names are sourced from the sdk operation registry', () => {
	...
	const sdkCommandNames = listOperationNames().sort();
